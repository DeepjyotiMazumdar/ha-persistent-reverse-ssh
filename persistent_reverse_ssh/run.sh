...
# Read options from HA add-on options file
...
# Build ssh command as a string, no bash arrays!
SSH_CMD="ssh -i \"$PRIVATE_KEY_PATH\" -p \"$REMOTE_SSH_PORT\" \
  -o ServerAliveInterval=$SERVER_ALIVE_INTERVAL \
  -o ServerAliveCountMax=$SERVER_ALIVE_COUNT_MAX \
  -o GatewayPorts=$GATEWAY_PORTS \
  $SSH_EXTRA_OPTIONS -N -R ${REMOTE_LISTEN_PORT}:${LOCAL_TARGET_HOST}:${LOCAL_TARGET_PORT} ${REMOTE_USER}@${REMOTE_HOST}"
...
if [ "$AUTOSSH_MONITOR_PORT" -eq 0 ]; then
    eval "exec autossh -M 0 $SSH_CMD"
else
    eval "exec autossh -M $AUTOSSH_MONITOR_PORT $SSH_CMD"
fi
